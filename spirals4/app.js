...
const s = (sketch) => {
    ...
    drawSomething = () => {
        ...
        xoff = xoff + 0.01;
        var distance = sketch.floor(index / 360) * 16;
        let n = sketch.noise(xoff) * (distance / 8);
        ...
        var x = p.x > 0 ? sketch.min(p.x, distance / 2) : sketch.max(p.x, distance / -2);
        var y = p.y > 0 ? sketch.min(p.y, distance / 2) : sketch.max(p.y, distance / -2);

        sketch.rect(x + n, y + n, 4);
